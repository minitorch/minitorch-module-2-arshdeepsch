...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        s = grad_output.f.sigmoid_map(t1)
        return grad_output.f.mul_zip(
            grad_output,
            grad_output.f.mul_zip(
                s,
                grad_output.f.add_zip(
                    t1._ensure_tensor(1.0), grad_output.f.neg_map((s))
                ),
            ),
        )
...
class ReLU(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (t1,) = ctx.saved_values
        zero = grad_output._ensure_tensor(0.0)
        return grad_output.f.mul_zip(grad_output, grad_output.f.lt_zip(zero, t1))
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        order_list = [int(order[i]) for i in range(order.size)]
        inv_order = np.argsort(order_list).tolist()
